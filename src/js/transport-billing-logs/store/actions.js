...
export const getTransportBillingLog = page => (dispatch, getState) =>
  axios.get('transport-billing-logs', {
    params: {
      page,
      from: getState().transportBillingLogReducer.from,
      to: getState().transportBillingLogReducer.to,
...
export const saveModel = model => (dispatch) => {
  if (model.id) {
    return axios.put(`transport-billing-logs/${model.id}`, {
      data: {
  ...
  return axios.post('transport-billing-logs', {
    data: {
      type: 'transport-billing-logs',
      attributes: model.attributes,
    },
    ...
    .then(() => {
      dispatch(getTransportBillingLog(1));
      dispatch(setEditableModel(null));
    });
};

export const deleteModel = id => dispatch =>
  axios.delete(`transport-billing-logs/${id}`)
    .then(() => {
