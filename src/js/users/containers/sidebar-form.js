...
import UserSidebarFormComponent from '../components/sidebar-form';
...
class UserSidebarForm extends CrudContainer {
  constructor(props) {
    super(props);
    this.state = {
      editableModel: null,
  ...
  getTitle() {
    ...
    if (this.props.userRole) {
      return `New ${this.props.userRole}`;
    }
    return 'New User';
  }
  ...
  handleSave() {
    ...
    if (!this.state.editableModel.attributes.email.length) {
      errors.push(Object.assign(
        {},
        validationErrorSkeleton,
        { detail: 'Email is required.', source: { pointer: 'data.attributes.email' } },
      ));
    }
    if (errors.length) {
...
UserSidebarForm.defaultProps = {
  editableModel: null,
  userRole: null,
  userRoles: null,
};

UserSidebarForm.propTypes = {
...
const mapStateToProps = ({ userReducer }) => ({
  isOpen: userReducer.editableModel !== null,
  editableModel: userReducer.editableModel,
});
